...
<title>Insert title here</title>
 <style>
  ...
  <script>
	   ...
	   for(i=0;i<ju;i++)
	   {
		   ...
		   for(j=0;j<7;j++)
		   {
			   ...
			   {   
				   ...
				   if(nalCheck(y,m,day)) // 오늘 이전의 날짜는 클릭이 되지 않는다
				   {
					   calData=calData+" <td class='td' onmouseover='over(0,this)' onmouseout='out(this)'  onclick='nalJaSend("+y+","+m+","+day+","+n+")'> "+day+" </td> ";
				   }	
				   else
				   {
					   calData=calData+" <td class='td' onmouseover='over(1,this)' onmouseout='out(this)'>"+day+" </td> ";
				   }	   
   ...
   function nalCheck(y,m,d)
   {
	   var today=new Date();
	   today=new Date(today.getFullYear(),today.getMonth(),today.getDate());
	   var xday=new Date(y,m,d);
	   console.log(xday.getTime()+" "+today.getTime());
	   if(xday.getTime()>=today.getTime())
		  	return true;
	   else
		   return false;
   }
...
<body>  <!-- javascript/cal7.jsp -->
 <h3> 달력 2달치 나와서 선택하기(오늘이전 선택불가)</h3>
 <div id="main">
